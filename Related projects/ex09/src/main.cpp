...
int main(int argc, char *argv[]) {
	try {
		...
		std::shared_ptr<SceneObjectEffect> phongshaded(new ShadedPhongEffect(glm::vec3(0.0, 0.0, 10.0)));

		std::shared_ptr<LightObject> light1(new PointLight(glm::vec3(5.0f, 10.0f, 5.0f)));
		rt->addLightSource(light1);
		std::shared_ptr<SceneObject> s1(new Sphere(glm::vec3(-3.0f, 0.0f, 6.0f), 2.0f, phongshaded));
